...
export class RedisCache {
  ...
  public async delete(namespace: string, key?: string): Promise<boolean> {
    try {
      ...
      if (key) {
        ...
        return results ? results.some(([err, result]) => !err && (result as number) > 0) : false;
      }
    } catch (err) {
  ...
  public async pipeline(operations: Array<[string, ...any[]]>): Promise<any[]> {
    try {
      await this.ensureConnection();
      const pipeline = this.client.pipeline();
  ...
  public async transaction(operations: Array<[string, ...any[]]>): Promise<any[]> {
    try {
      await this.ensureConnection();
