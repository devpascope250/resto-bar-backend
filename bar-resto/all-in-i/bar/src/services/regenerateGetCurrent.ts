...
import { StockManagement, stockOutData } from "./StockManagement";
...
export class RegenerateGetCurrent {
    async createNewReceipt(headers: any, body: Sales, userId: string, increment?: number) {
        const salesData = body; try {
            ...
            const apiPromises = preparedCustomers.map(async (data, index) => {
                ...
                const salesT = {
                    invcNo: Number(latestInvoiceId) + index + 1 + (increment || 0),
                    orgInvcNo: 0,
            ...
            await prisma.$transaction(async (tx) => {
                ...
                for (const [index, data] of preparedCustomers.entries()) {
                    ...
                    const invcNoCounter = Number(latestInvoiceId) + index + 1 + (increment ?? 0);
            ...
            throw { resultCd: 500, message: (error as Error).message ?? (error as any).resultMsg ?? "Error while generating invoices" };
        }
    ...
    async getReceipts(headers: any, body: any, increment?: number) {
        ...
        try {
            ...
            for (let i = 1; i <= getallInvo.length; i++) {
                ...
                expectedNewInvId.push({ invcNo: Number(latestInvoiceId) + 1 + (increment || 0), orderCustomerId: orderCustomerId });
            }
